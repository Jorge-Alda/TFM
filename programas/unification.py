"""
Calculates the unification scale and unification distance for points in the (ms, Ns) parameter space
...
for i in range(0,20000):
	ms = np.exp(random.uniform(np.log(10), np.log(10)*4))
	Ns = random.randint(2,150)
	...
	# Calculate unification scale and unification distance
	GUTscale = tarray[min(enumerate(dist), key=itemgetter(1))[0]]
	maxl = max(max(lharray), -min(lharray), max(lsarray), -min(lsarray), max(lhsarray), -min(lhsarray) )

    # Save data in file if the couplings remain perturbative
	if maxl < 3.0:
		f = open('data2.txt', 'a')
		f.write('{0}\t{1}\t{2}\t{3}\t{4}\t{5}\t{6}\t{7}\t{8}\n'.format(ms, Ns, ls, lhs, vs, maxl, min(dist), GUTscale, theta))
		f.close()
